 ...
 */
public class SAPLowComputation {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for (FogDevice device : fogDevices) {
                ...
                } else if (device.getName().startsWith("fog")) {
...
//                FogDevice msf = (FogDevice) f;
//                if (msf.getDeviceType() == FogDevice.FON || msf.getDeviceType() == FogDevice.CLOUD) {
//                    List<FogDevice> fogDevices = new ArrayList<>();
//                    fogDevices.add(f);
//                    monitored.put(f.getId(), fogDevices);
//                    msf.setFonID(f.getId());
//                } else if (msf.getDeviceType() == FogDevice.CLIENT) {
...
//            CloudSim.stopSimulation();
    ...
    private static void createFogDevices(int userId, String appId) {
        FogDevice cloud = createFogDevice("cloud", 44000, 40000, 10000, 10000, 0, 0.01, 1332, 1648); // creates the fog device Cloud at the apex of the hierarchy with level=0
        cloud.setParentId(-1);
    ...
    private static FogDevice addGw(String id, int userId, String appId, int parentId) {
        FogDevice area = createFogDevice("d-" + id, 2500, 2000, 1000, 10000, 1, 0.0, 107.339, 83.4333);
        fogDevices.add(area);
    ...
    private static FogDevice addEdgeServer(String id, int userId, String appId, int parentId) {
        ...
        FogDevice edgeServer = createFogDevice("fog-" + id, mips, ram, 5000, 10000, 3, 0, 83.43, idlePower);
        edgeServer.setParentId(parentId);
        return edgeServer;
    }

    private static FogDevice addTerminal(String name, int userId, int parentId) {
        FogDevice mobile = createFogDevice(name, 1000, 1500, 3000, 3000, 4, 0, 60.43, 87.43);
